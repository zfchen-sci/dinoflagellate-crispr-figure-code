...
def plot(source_dir: Path, output_dir: Path) -> None:
    configure_style()
    source = source_dir / "Source_data_fig5.xlsx"
    annotation = pd.read_excel(source, sheet_name="Fig.5a")
    sxta = pd.read_excel(source, sheet_name="Fig.5b")
    pst = pd.read_excel(source, sheet_name="Fig.5c")
    deg = pd.read_excel(source, sheet_name="Fig.5d")
    persistent = pd.read_excel(source, sheet_name="Fig.5e")
    ...
    for offset, group, color in ((-width / 2, WT, COLORS["wt"]), (width / 2, EDITED, COLORS["edited"])):
        ...
        ax.bar(x + offset, means, width, yerr=sds, capsize=2, color=color, edgecolor=COLORS["dark"], label=DISPLAY[group])
        for i, day in enumerate(days):
            values_at_day = sxta.loc[(sxta["group"] == group) & (sxta["day"] == day), "fpkm"].to_numpy()
            ax.scatter(x[i] + offset + np.linspace(-0.04, 0.04, len(values_at_day)), values_at_day, s=8, color=COLORS["dark"], zorder=3)
    raw_p = [
        stats.ttest_ind(
            sxta.loc[(sxta["day"] == day) & (sxta["group"] == WT), "fpkm"],
            sxta.loc[(sxta["day"] == day) & (sxta["group"] == EDITED), "fpkm"],
            equal_var=False,
        ).pvalue
        for day in days
    ]
    ...
    for i, (day, p_value) in enumerate(zip(days, adjusted)):
        subset = sxta[sxta["day"] == day]
        fold = subset.loc[subset["group"] == WT, "fpkm"].mean() / subset.loc[subset["group"] == EDITED, "fpkm"].mean()
        y = subset["fpkm"].max() * 1.08
        add_bracket(ax, x[i] - width / 2, x[i] + width / 2, y, significance_label(float(p_value)), height=2)
        ax.text(x[i], y + 8, f"{fold:.1f}×", ha="center", va="bottom")
    ax.set(xticks=x, xticklabels=[f"{day}d" for day in days], xlabel="Time", ylabel=r"$sxtA$ expression (FPKM)", ylim=(0, 150))
    ...
    adjusted = multipletests([
        stats.ttest_ind(
            pst.loc[(pst["pst_gene"] == gene) & (pst["group"] == WT), "fpkm"],
            pst.loc[(pst["pst_gene"] == gene) & (pst["group"] == EDITED), "fpkm"],
            equal_var=False,
        ).pvalue
        for gene in genes
    ], method="holm")[1]
    for offset, group, color in ((-width / 2, WT, COLORS["wt"]), (width / 2, EDITED, COLORS["edited"])):
        ...
        ax.bar(x + offset, means, width, yerr=sds, capsize=1.5, color=color, edgecolor=COLORS["dark"], label=DISPLAY[group])
    ax.set_yscale("symlog", linthresh=1)
    for i, (gene, p_value) in enumerate(zip(genes, adjusted)):
        subset = pst[pst["pst_gene"] == gene]
        y = max(subset["fpkm"].max() * 1.15, 0.25)
        add_bracket(ax, x[i] - width / 2, x[i] + width / 2, y, significance_label(float(p_value)), height=max(y * 0.08, 0.05))
    ...
    ax.set(xticks=x, xticklabels=[str(value) for value in deg.iloc[:, 0]], xlabel="Time (d)", ylabel="Number of DEGs")
    ax.legend(loc="upper left")
    panel_label(ax, "d")
    ...
    colorbar.set_label(r"log$_2$ fold change edited/WT")
    ax.set_title(f"{len(persistent):,} shared DEGs; {int(persistent['dark_gene'].sum())} unannotated")
    panel_label(ax, "e")

    save_figure(fig, output_dir, "Figure5_data_panels")


def main() -> None:
    parser = argparse.ArgumentParser(description="Recreate data-driven panels for current Figure 5.")
